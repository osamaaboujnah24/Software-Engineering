...
class StudentMessageHandler {
    ...
    public function sendReply($receiverId, $message) {
        ...
        if (empty($message)) {
            $this->feedback = "<p style='color:red;'> لا يمكن إرسال رد فارغ.</p>";
            return;
        }

        if (!$this->project) {
            $this->feedback = "<p style='color:red;'> لا يوجد مشروع مرتبط بك.</p>";
            return;
        }
        ...
        $this->feedback = "<p style='color:green;'> تم إرسال الرد.</p>";
    }
}
...
<?php if ($project): ?>
        ...
        <?php foreach ($handler->messages as $msg): ?>
            <div style="border:1px solid #ccc; padding:10px; margin-bottom:10px;">
                ...
                <form method="POST">
                    ...
                    <textarea name="reply_message" rows="2" cols="50" placeholder="اكتب ردك هنا..." required></textarea><br>
    ...
    <?php endif; ?>
<?php else: ?>
    <p style='color:red;'> لا يوجد مشروع مرتبط بك.</p>
<?php endif; ?>
